...
public class LocalLLMMNNHandler implements LocalLlmHandler.InferenceEngine {
    ...
    private void initializeLLM(String modelPath, LocalLlmHandler.ModelConfig config) throws Exception {
        ...
        // Build MNN session config and inject enable_thinking into mnnConfig.
        // CRITICAL: config_runtime_merged.json is generated by merging config.json + mnnConfig at createSession time.
        // MNN reads enable_thinking from this file during createLLM()/load() to compile the jinja template.
        // updateConfig() after load() does NOT affect the jinja template - it is too late.
        // Therefore enable_thinking MUST be set here at model load time.
        ...
        try {
            ...
            if (jinjaObj == null) { jinjaObj = new org.json.JSONObject(); configObj.put("jinja", jinjaObj); }
            org.json.JSONObject contextObj = jinjaObj.optJSONObject("context");
            if (contextObj == null) { contextObj = new org.json.JSONObject(); jinjaObj.put("context", contextObj); }
            contextObj.put("enable_thinking", thinkingEnabled);
            mnnConfig = configObj.toString();
            LogManager.logI(TAG, "[INIT] enable_thinking=" + thinkingEnabled + " injected into mnnConfig (noThinking=" + (runtimeConfig != null ? runtimeConfig.noThinking : "null") + ")");
        } catch (org.json.JSONException e) {
            LogManager.logW(TAG, "[INIT] Failed to inject enable_thinking: " + e.getMessage());
        }
        
        // Create MNN session - config_runtime_merged.json will be written with enable_thinking above
    ...
    // ========== Configuration Change Detection ==========
        ...
        // ========== Configure thinking mode (Qwen3/Qwen3.5) ==========
        // enable_thinking is set here via updateConfig() before each inference.
        // Qwen3/Qwen3.5 both use jinja.context.enable_thinking to control thinking mode.
        // This is the ONLY place to set it - NOT in initializeLLM() which may use stale config.
        ...
        try {
            MnnInference.updateConfig(llmSessionHandle,
                String.format("{\"jinja\":{\"context\":{\"enable_thinking\":%s}}}", thinkingEnabled));
            LogManager.logI(TAG, "[THINKING] enable_thinking=" + thinkingEnabled + " (noThinking=" + (cfgForThinking != null ? cfgForThinking.noThinking : "null") + ")");
        } catch (Exception e) {
