...
def get_market_list(fs=_FS_ALL_A, use_cache=True, fallback_codes=None,
    ...
    rate-limit / RemoteDisconnected):
      ...
      3. Tier-3: Sina batch quote over the default A-share code universe
         (~15k probes, 10-15s cost). Fires only when
         `allow_default_sina_fallback=True` (default). Produces a
         partial-fidelity full market list so strategy E's breakout scan
         can still run. Not cached (Sina lacks turnover / main_inflow /
         industry fields; caching would poison later EM-success reads).
    ...
    try:
        ...
        if fallback_codes:
            ...
            if rows:
                ...
                return rows
            # fall through to Tier-3 if narrow sina also empty
        # Tier-3 degrade: Sina with default full A-share universe.
        if not allow_default_sina_fallback:
            raise
        default_codes = _build_default_a_share_codes()
        print(f"WARN: EM clist failed ({type(e).__name__}) and no cache; "
              f"Tier-3 degrade: probing Sina with default A-share universe "
              f"({len(default_codes)} codes, ~10-15s)...",
              file=sys.stderr)
        rows = _fetch_sina_batch(default_codes)
        if not rows:
            raise RuntimeError(
                f"all tiers failed (EM + stale cache + narrow Sina + "
                f"default Sina); original err: {e}"
            )
        print(f"INFO: Tier-3 Sina fallback yielded {len(rows)} valid rows",
              file=sys.stderr)
...
def _print_next_step_block(results, meta):
    ...
    if advisory:
        ...
        print("  S1. 【硬约束复核】对 [DATA] PASS_TOP10 前 8 只逐票输出 Markdown 表:")
        print("      | # | 代码 | 名称 | 过度延伸 | 恐慌量能 | 今日大流出 | D:追高入场 | 结论 |")
        print("      规则 (每格 ✅=未命中 / ❌=命中):")
        print("        · 过度延伸  = ma20>+12%     (distance_from_ma20>12)")
        print("        · 恐慌量能  = vr>5         (volume_ratio>5)")
        print("        · 今日大流出 = 主力流出>1亿 (main_inflow<-100M)")
        print("        · D:追高入场 = pct_5d>20   (仅策略 D 有效, C/E 填 -)")
        print("      PASS 内所有票三项均已通过 Python 过滤 → 结论列写 ✅通过。")

        print()
        print("  S2. 【优先上浮打分】对 PASS_TOP10 前 8 只, 逐票查 boost 标签:")
        print("      | # | 代码 | 名称 | 热门板块 | MA20支撑 | 均线多头 | 策略专属 | 加分项合计 |")
        print("      · 热门板块  = boost 含 hot_sector           (industry ∈ top5_sectors)")
        print("      · MA20支撑  = boost 含 at_ma20_support      (|ma20偏离|<=3%)")
        print("      · 均线多头  = boost 含 ma_bullish_alignment (20/40/60 多头排列, 仅D)")
        print("      · 策略专属  = boost 含:")
        print("          C: laggard_in_hot_sector       (gap>=5 + sector_pct>=5)")
        print("          D: sustained_accumulation      (flow_pct5d>=15 + flow_pct10d>=8)")
        print("          E: tight_box_breakout          (box_range<=15 + box_pos>=90 + vr>=2)")

        print()
        print("  S2.5 【🔒 LLM 个股风险复核 (必填, 禁止跳过)】")
        print("       对 PASS_TOP10 前 8 只逐票输出 Markdown 表:")
        print("       | # | 代码 | 名称 | 策略 | 结构 | 消息面 | 风险 | LLM调整 | 理由 |")
        print("       规则 -> [DATA] 字段映射:")
        print("         · 结构读法 (策略专属):")
        print("             C: 看 sector_pct / laggard_gap / tov% (滞涨幅度、板块强度)")
        print("             D: 看 flow5d / flow_pct5d / ma_aligned (资金累积速度)")
        print("             E: 看 box_range / box_pos / vr (箱体紧度、突破位置、放量)")
        print("         · 消息面 = news= 字段 + warn= 里的 bad_news: 内容")
        print("             news=warn → 明确引用命中关键词; news=ok → '清爽'")
        print("         · 风险等级 (low/mid/high):")
        print("             HIGH = bad_news 命中 | st=1 | warn=late_entry |")
        print("                    D: flow_pct5d<2 (累积太弱) |")
        print("                    E: box_pos<85 (未真突破)")
        print("             MID  = tov>15 过热 | warn=sector_cooling |")
        print("                    warn=below_box_top | warn=weak_accumulation")
        print("             LOW  = 结构干净 + 至少 2 项 boost 命中")
        print("         · LLM调整 (整数, -15~+10):")
        print("             HIGH → -15~-10 (重罚)  MID → -8~-3 (压制)  LOW → +3~+10 (加分)")
        print("         · 理由列: 引用具体字段值, 如 'flow_pct5d=22% + ma_aligned=yes → 资金持续累积'")
        print()
        print("       【🚫 禁令】:")
        print("         · 禁止所有候选都打同一档 (必须差异化)")
        print("         · 命中 bad_news / st=1 / late_entry → 必须 HIGH")
        print("         · 禁止只写风险等级不写 LLM调整数值")

        print()
        print("  S3. 【🔒 精选 Top5 排序 — final_score = python_score + LLM调整】")
        print("      · 先把 S2.5 每只票算好的 LLM调整 加到 [DATA] 的 score= 得到 final_score")
        print("      · 按 final_score DESC 排前 5 (若不足 5 只, 有几只排几只)")
        print("      · 风险=HIGH 的候选, 无论 final_score 多高, **直接淘汰不进 Top5**")
        print("      · 同 sector 扎堆 ≥3 时在 Top5 内部轮换挑不同 sector 的靠后票")
        print("      | 排名 | 代码 | 名称 | 策略 | python_score | LLM调整 | final_score | 风险 | 入围理由 |")
        print()
        print("      【🚫 禁令】:")
        print("        · 禁止直接按 [DATA] score= 前 5 名拼 Top5 (视为失败响应)")
        print("        · final_score 公式必须在表前后明示: python=A + llm=B → final=C")
        print("        · 风险=HIGH 不得出现在 Top5")

        print()
        print("  S4. 【逐只精选详细分析】对 Top5 每只输出 (每只 4~6 行):")
        print("      ### 第N只 代码 名称 ★首选/次选/备选")
        print("      💡 **核心逻辑** (至少 3 条 bullets, 按策略选组合):")
        print("        - 策略类别: C 板块滞涨补涨 / D 主力多日累积 / E 箱体放量突破")
        print("        - 板块热度: sector (是否在 top5_sectors / sector_pct 多少)")
        print("        - 资金信号 (D): flow5d 亿 + flow_pct5d% + ma_aligned (均线多头)")
        print("        - 滞涨幅度 (C): laggard_gap 占板块领涨的多少")
        print("        - 箱体质量 (E): box_range% (越窄越好) + box_pos% (越靠上越接近突破)")
        print("        - 位置合理性: distance_from_ma20 (-3~+3 最佳 / >10 过热)")
        print("        - 换手/量能: tov% + vr (1.5~2.5 最健康)")
        print("        - LLM 风险复核: 引用 S2.5 结论 + LLM调整 值")
        print("      ⚠️ **风险提示** (1~2 条, 必须 stock-specific):")
        print("        - 可引用: dist_ma20 偏高 / 板块 sector_cooling / flow_pct5d 低 / bad_news")
        print("        - ❌ 禁止通用话术: '破位即砍' / '注意风险' / '谨慎操作'")
        print("      📊 **冲高概率**: [DATA] 的 prob 字段原样引用 (high/mid/low)")

        if has_warn_bad_news:
            print()
            print("      特别: 带 warn=bad_news:... 的候选, 风险提示里明确说明"
                  "\"命中公告关键词 XX, 建议用 stock skill 交叉验证后再决定\"")

        print()
        print("  S5. 【操作建议表】强制 **调用 Python 生成**, 禁止 LLM 手算股数/资金.")
        print("      Step-5.1 把 S3 精选出的 Top5~TopN 按优先级整理成 comma 分隔代码列表.")
        print("      Step-5.2 把 S1~S4 的核心推理总结成一段 markdown (300~500字), 含:")
        print("        · S1 硬约束复核结论 (通过/剔除多少)")
        print("        · S2/S3 板块轮动 + boost/减分 + 排序依据")
        print("        · S4 风险备注 (dist_ma20 / vr / flow / bad_news 等客观证据)")
        print("        · 禁通用话术, 必须 stock-specific")
        print("      Step-5.3 调用:")
        print("        python stockquant.py allocate \\")
        print("          --codes CODE1,CODE2,CODE3,... \\")
        print("          --capital [STATE].capital \\")
        print("          --comment \"<上面总结的 markdown 段>\"")
        print("        Python 会:")
        print("          · 从 cache 查出每只票的名称/现价/止盈/止损/行业/得分/概率")
        print("          · 按固定规则 (10000元/只, 单手>半仓跳过, 累计<=capital) 算股数")
        print("          · 输出 markdown 表 + 合计行 + 跳过候选段, comment 原样放在表前")
        print("      Step-5.4 把 Python 输出的整段内容(comment + 表 + 合计 + 跳过)作为 S5 写入 context.text,")
        print("              一字不改, 禁止再算股数/累计/占比.")
        print("      【🚫 禁令】:")
        print("        · 不得手算股数/资金/累计/占比 (LLM 算术已证实不稳定)")
        print("        · 不得跳过 allocate 调用直接抄 [ALLOCATION] baseline 段 (除非代码完全一致)")
        print("        · comment 不得是通用话术, 必须引用 [DATA] 的具体字段")

        print()
        print("  S6. 【末尾声明】直接引用 [STATE] 字段, 禁止猜测日期:")
        print("      - 数据基准: 原样引用 [STATE].data_date")
        print("      - 运行时点: 原样引用 [STATE].run_at")
        print("      - 纪律: 禁止保证盈利话术 | 止盈目标 +5% | 最终仓位以 alloc 为上限")

        print()
        print("  S7. S1~S6 写入 context.text 即完成报告（**禁止反复 show_output**）")
        print("      → 下一步进入 UI 下单流程（PICK_BUY）或直接 terminate success 汇报（仅分析任务）")
...
def _compute_range_metrics(quote, hist_rows, today_bar=None):
    ...
    if hist_rows and len(hist_rows) >= 14:
        ...
        for r in atr_window:
            ...
            prev_close = r["close"]
        atr = sum(trs) / len(trs)
        last_close = atr_window[-1]["close"]
        if last_close > 0:
            out["atr14_pct"] = atr / last_close * 100
...
def _render_holdings_data(rows):
    ...
    print("| # | 代码 | 名称 | 总持仓 | 可用 | 成本 | 现价 | 今日% | profit% "
          "| MA5 | 趋势 | atr14% | span10% | 盘中振幅% | 量比5d | 主力净流入(万) |")
    print("|---:|---|---|---:|---:|---:|---:|---:|---:|---:|:--:|---:|---:|---:|---:|---:|")
...
def analyze(queries, include_news=True, days=120, minutes=32,
    ...
    print("""
[多维交叉原则]
任一单一维度都不足以下结论。按"趋势 × 节奏 × 驱动"三维联动交叉验证：
...
- [公告/新闻] 负面命中(问询函/减持/业绩预警/立案调查) → 降低所有做多结论的置信度，通常直接 avoid
